!---------------------------------------------------------------------------------------!
!                                                                                       !
! To count number of GCs which is a function of redshift: N_GCs(z)                      !
! (formation redshift distribution of primordial GCs)                                   !
!---------------------------------------------------------------------------------------!
...
implicit none
  ...
  real, allocatable        :: wlev(:),alev(:), z_array(:), mphalo_array(:)
  integer, allocatable     :: ifraglev(:), GC_mass_array(:), GC_z_array(:)
  ...
  integer                  :: count_branch, count_GC, jlevelmax, n_z_bin, n_mass_bin, k, i_count, z_form_GC_lev
...
! generate an array of M_halo at z=0, [M_sun] to compare to Creasey, then take M_halo*h0 <-> [M_sun/h] to put into code 
 ...
 mphalo_max   = 1.0e+13
 n_mass_bin   = 6
 allocate(mphalo_array(n_mass_bin))
 mphalo_array = (/mphalo_min, 1.0e+9, 1.0e+10, 1.0e+11, 1.0e+12, mphalo_max/)
 ...
 z_form_GC_lev = 173
 n_z_bin   = nlev - z_form_GC_lev
 allocate(GC_z_array(n_z_bin), z_array(n_z_bin)) 
 call linspace(z_form_GC,z_max,z_array)
...
! Set primordial P(k) parameters (ignored if itrans=-1)
 nspec  = ns_COCO              !primoridial power spectrum spectral index
 dndlnk = 0.0                  !allow running spectral index by setting ne.0
 kref   = 1.0                  !pivot point for running index
 sigma8 = sigma8_COCO          !power spectrum amplitude set regardless of other parameters, default = 0.9
...
do i = 1, ntree
    ...
    do while (i_count .le. n_mass_bin) 
       ...
       write(0,*)'made tree',i, 'with M(z=0) = 1e',int(log10(mphalo_array(i_count))),'[M_sun]'
       ...
       do while (associated(This_Node))
          if (This_Node%mhalo .le. M_form_GC .and. This_Node%parent%mhalo .gt. M_form_GC) then
              !z = 1.0/alev(This_Node%parent%jlevel) - 1.0
              !if (z .ge. z_form_GC) then
                  !k = nint((z-z_form_GC)/z_step) + 1
              if (This_Node%parent%jlevel .ge. z_form_GC_lev) then
                  k = This_Node%parent%jlevel - z_form_GC_lev + 1
                  GC_z_array(k) = GC_z_array(k) + 1         
              endif
       ...
       write(f1,*) i
       write(f2,*) int(log10(mphalo_array(i_count)))

       open(unit = 101,file = 'results_z/GCs-tree'//trim(adjustl(f1))//'M=1e'//trim(adjustl(f2))//&
               &'-func-of-z-COCO.txt',status='replace') 
       do k = 1, n_z_bin 
           write (101,*) z_array(k), GC_z_array(k)     !mhalo_array [M_sun] to compare to Creasey    
       enddo
       close(101)
