...
!------------------------------------------------------------------!
 ...
 z_form_GC = 8.65
 M_form_GC = 1.0e+08*h0    ! unit [M_sun/h0] to compare to M_halo inside the code 
...
do i = 1001, ntree
    ...
    do while (i_count .le. n_mass_bin) 
       ...
       do while (associated(This_Node))
          if (This_Node%mhalo .ge. M_form_GC) then
   ...
   enddo
    ...
    write(f1,*) i
    open(unit = 101,file = 'results_masses/GCs-tree'//trim(adjustl(f1))//'-func-of-mass-COCO-test.txt',status='replace') 
    do k = 1, n_mass_bin 
