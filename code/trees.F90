...
implicit none
  ...
  integer                  :: count_branch, count_GC, jlevelmax, n_z_bin, kk, ii
...
! These cosmological parameters are taken from Mill
 omega0  = 0.25 
 lambda0 = 0.75
 h0      = 0.73
 omegab  = 0.04
...
!     print *, 'nlevel', jlevelmax
     ...
     !save in file the number count and redshift of halos which form GC
     !write(filename,*) i
     !open(unit = 101,file = 'test_results/GCs-in-tree'//trim(adjustl(filename))//'test2.txt' ,status='replace') 
     This_Node => MergerTree(1)
     do while (associated(This_Node))
        if (This_Node%mhalo .lt. M_form_GC .and. This_Node%parent%mhalo .ge. M_form_GC) then
            z = 1.0/alev(This_node%parent%jlevel)-1.0
     ...
     print *, count_GC_array
     !do ii = 1, n_z_bin
         !write (101,*) z_form_GC + (ii-1)*z_step, count_GC_array(ii)
     !enddo
     !close(101)

     !--------------------------------------------------------------!
     ! Number of GCs as function of halo mass M(z=0)                !
     !--------------------------------------------------------------!
